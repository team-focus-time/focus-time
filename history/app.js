...
function _loadFt(){ try { return JSON.parse(localStorage.getItem('ft_tasks')||'[]'); } catch(e){ return []; } }
...
function renderTasksForDate(ymd, filter='all'){
  ...
  filtered.forEach(t=>{
    ...
    div.className = 'task';
    div.innerHTML = `<div class="left"><div class="time">${t.time}</div><div class="title ${t.done ? 'completed' : ''}">${t.title}</div><div class="mins">${t.mins}m</div></div>
...
function renderAllTasks(filter='all'){
  ...
  const filtered = allTasks.filter(t=> filter==='all' ? true : (t.tag===filter));
  if(filtered.length===0){
  ...
  filtered.forEach(t=>{
    ...
    el.className = 'task';
    el.innerHTML = `<div class="left"><div class="time">${t.time}</div><div class="title ${t.done ? 'completed' : ''}">${t.title}</div><div class="mins">${t.mins}m</div></div>
      <div class="bar"><i style="width:${pct}%"></i></div>
      <div class="tag ${t.tag}">${t.tag}</div>`;
...
function renderTasksForDate(ymd, filter='all'){
  ...
  filtered.forEach(t=>{
    ...
    row.innerHTML = `
      ...
      <div class="title ${t.done ? 'completed' : ''}" title="${t.title || ''}">${t.title || 'Untitled'}</div>
...
(function(){
  ...
  function periodRange(period, refDate=new Date()){
    ...
    if(period==='day'){
      const start=new Date(d.getFullYear(),d.getMonth(),d.getDate()); const end=new Date(start); end.setDate(start.getDate()+1);
      return { start, end };
    }
    if(period==='week'){
    ...
    // month
    const start = new Date(d.getFullYear(), d.getMonth(), 1); const end = new Date(d.getFullYear(), d.getMonth()+1, 1);
    return { start, end };
  }
  ...
  function renderMiniCalendar(container, year, monthIndex, goalMap){
    ...
    let html = `<div style="font-weight:800;color:#122168;margin-bottom:10px">${monthIndex+1}월<br><small style="color:#6b6f85;font-weight:600">${year}</small></div>`;
  ...
  function renderTodayTasks(container, data, period='day', tagFilter='all'){
    const { tasks } = data;
    ...
    const filtered = tasks.filter(t=>{
      ...
      return dt >= pr.start && dt < pr.end && (tagFilter==='all' || (t.tag===tagFilter));
    });
  ...
  function renderDonut(container, data, period='month'){
    ...
    parts.forEach(p=>{
      ...
      html += `<circle class="donut-seg" r="15.9" cx="21" cy="21" stroke="${TAGS[p.tag].stroke}" stroke-width="8"
                 stroke-dasharray="${arc} ${100-arc}" transform="rotate(-90 21 21)" style="stroke-dashoffset:${-offset}"></circle>`;
      offset += arc;
    });
    html += `</svg></div>`;
    // legend
    ...
    parts.forEach(p=>{
      ...
      html += `<div class="legend-row"><span class="legend-swatch" style="background:${TAGS[p.tag].stroke}"></span><strong>${TAGS[p.tag].label}</strong> <small style="color:#6b6f85">${p.mins}m • ${pct}%</small></div>`;
    });
    html += `</div></div>`;
    container.innerHTML = html;
  }
  ...
  function renderFocusBars(container, data, period='week'){
    ...
    html += `</div>`;
    container.innerHTML = `<div style="display:flex;justify-content:space-between;align-items:flex-start"><h3 style="margin:0;color:#122168">전체 집중한 시간</h3><div style="color:#122168;font-weight:700">주간, 월간, 연간</div></div>` + html;
  }
  ...
  window.openChartPanel = function(){
    ...
    const targetMinsPerDay = parseInt(localStorage.getItem('targetMinsPerDay')||'60',10);
    const year = (new Date()).getFullYear();
    ...
    const goalMap = {};
    Object.keys(daily).forEach(k => goalMap[k] = Math.round( (daily[k]/targetMinsPerDay) * 100 ));

    // render sections
    const mini = RIGHT.querySelector('#miniCalendarCard'); renderMiniCalendar(mini, year, monthIndex, goalMap);
    const todayCard = RIGHT.querySelector('#todayTasksCard'); renderTodayTasks(todayCard, data, 'day', 'all');
    const goalCard = RIGHT.querySelector('#goalCard'); goalCard.innerHTML = `<div style="font-weight:800;color:#122168">목표 달성 시간</div><div style="color:#6b6f85;margin-top:8px">목표: ${Math.round(targetMinsPerDay/60)}h / day</div>`;

    const focusCard = RIGHT.querySelector('#focusBarsCard'); renderFocusBars(focusCard, data, 'week');
    const donut = RIGHT.querySelector('#donutCard'); renderDonut(donut, data, 'month');
    ...
    RIGHT.querySelector('#panelRefresh').addEventListener('click', ()=> {
      ...
      const d = loadData();
      renderMiniCalendar(RIGHT.querySelector('#miniCalendarCard'), year, monthIndex, goalMap);
    ...
    RIGHT.querySelector('#panelClose').addEventListener('click', ()=> {
      ...
      RIGHT.innerHTML = originalRightHTML;
      RIGHT.classList.remove('replaced-right');
      setTimeout(()=>{ if (typeof mountGrid === 'function') mountGrid(); if (typeof updateMiniCardProgress === 'function') updateMiniCardProgress(); }, 60);
    });
    ...
    RIGHT.addEventListener('click', function handler(e){
      ...
      renderTodayTasks(RIGHT.querySelector('#todayTasksCard'), loadData(), period, 'all');
    });
  };

  // wire chart button if present
  document.addEventListener('click', (e)=>{
    if (e.target && e.target.id === 'chartBtn') { e.preventDefault(); openChartPanel(); }
  });
})();
...
(function(){
  ...
  window.addEventListener('ft-tasks-updated', (ev)=> {
    ...
    // 기존 업데이트 루틴가 있으면 호출
    try { if (typeof updateMonthProgressFromTasks === 'function') updateMonthProgressFromTasks(tasks); } catch(e){}
    try { if (typeof updateTodayTasksCard === 'function') updateTodayTasksCard(tasks, 'day'); } catch(e){}
    try { if (typeof updateChartPanel === 'function') updateChartPanel(tasks,'month'); } catch(e){}
  });
  ...
  window.addEventListener('storage', (e)=>{
    if (e.key === 'ft_tasks') {
      ...
      } catch(err){ console.error('[History] storage parse', err); }
    }
  });
  ...
  (function timerModule(){
    const el = document.getElementById('timer') || document.querySelector('.timer-display');
    if (!el) return;
    const startBtn = document.getElementById('btnStart') || document.querySelector('[data-action="timer-start"]');
    const stopBtn = document.getElementById('btnStop') || document.querySelector('[data-action="timer-stop"]');
    const resetBtn = document.getElementById('btnReset') || document.querySelector('[data-action="timer-reset"]');
    ...
    function reset(){
      state = {running:false, startedAt:0, acc:0, targetMins: state.targetMins};
      save();
      if (raf){ cancelAnimationFrame(raf); raf = null; }
      render();
    }
  ...
  // Stopwatch (counts up)
  (function swModule(){
    const el = document.getElementById('stopwatch') || document.querySelector('.stopwatch-display');
    if (!el) return;
    const startBtn = document.getElementById('swStart') || document.querySelector('[data-action="sw-start"]');
    const stopBtn = document.getElementById('swStop') || document.querySelector('[data-action="sw-stop"]');
    const lapBtn = document.getElementById('swLap') || document.querySelector('[data-action="sw-lap"]');
    const resetBtn = document.getElementById('swReset') || document.querySelector('[data-action="sw-reset"]');
    ...
    function load(){ try{ const s = JSON.parse(localStorage.getItem(KEY)); if (s) state = Object.assign(state,s); }catch(e){} }
    function save(){ try{ localStorage.setItem(KEY, JSON.stringify(state)); }catch(e){} }
    function getElapsed(){ const now=Date.now(); return state.acc + (state.running ? (now - state.startedAt) : 0); }
    function render(){ if (el) el.textContent = _formatMS(getElapsed()); }
    function loop(){ render(); if (state.running) raf = requestAnimationFrame(loop); }
    function start(){ if (state.running) return; state.running=true; state.startedAt=Date.now(); save(); if (!raf) loop(); }
    function stop(){ if (!state.running) return; state.acc += (Date.now()-state.startedAt); state.running=false; state.startedAt=0; save(); if (raf){ cancelAnimationFrame(raf); raf=null; } render(); }
    function reset(){ state = {running:false, startedAt:0, acc:0, laps:[]}; save(); if (raf){ cancelAnimationFrame(raf); raf=null; } render(); }
    function lap(){ state.laps = state.laps || []; state.laps.push(getElapsed()); save(); }
...
(function ensureLeftWidgetsInit(){
  ...
  function bindTimerFallback(){
    ...
    let state = { running:false, startedAt:0, acc:0, targetMins:60 };
    let raf = null;
    function remaining(){ return Math.max(0, state.targetMins*60000 - (state.acc + (state.running ? (Date.now()-state.startedAt) : 0))); }
    function render(){ if (el) el.textContent = _fmtMS(remaining()); }
    function loop(){ render(); if (state.running) raf = requestAnimationFrame(loop); }
    function start(){ if (state.running) return; state.running=true; state.startedAt=Date.now(); if (!raf) loop(); }
    function stop(){ if (!state.running) return; state.acc += (Date.now() - state.startedAt); state.running=false; state.startedAt=0; if (raf){ cancelAnimationFrame(raf); raf=null; } render(); }
    function reset(){ state = { running:false, startedAt:0, acc:0, targetMins: state.targetMins }; if (raf){ cancelAnimationFrame(raf); raf=null; } render(); }
    if (startBtn) startBtn.addEventListener('click', ()=> { if(state.running) stop(); else start(); });
    if (stopBtn) stopBtn.addEventListener('click', ()=> stop());
    if (resetBtn) resetBtn.addEventListener('click', ()=> reset());
    // expose
  ...
  function bindStopwatchFallback(){
    ...
    let state = { running:false, startedAt:0, acc:0, laps:[] };
    let raf = null;
    function elapsed(){ return state.acc + (state.running ? (Date.now()-state.startedAt) : 0); }
    function render(){ if (el) el.textContent = _fmtMS(elapsed()); }
    function loop(){ render(); if (state.running) raf = requestAnimationFrame(loop); }
    function start(){ if (state.running) return; state.running=true; state.startedAt=Date.now(); if (!raf) loop(); }
    function stop(){ if (!state.running) return; state.acc += (Date.now()-state.startedAt); state.running=false; state.startedAt=0; if (raf){ cancelAnimationFrame(raf); raf=null; } render(); }
    function lap(){ state.laps.push(elapsed()); }
    function reset(){ state = { running:false, startedAt:0, acc:0, laps:[] }; if (raf){ cancelAnimationFrame(raf); raf=null; } render(); }
    if (startBtn) startBtn.addEventListener('click', ()=> start());
    if (stopBtn) stopBtn.addEventListener('click', ()=> stop());
    if (lapBtn) lapBtn.addEventListener('click', ()=> lap());
    if (resetBtn) resetBtn.addEventListener('click', ()=> reset());
    window.ftStopwatch = window.ftStopwatch || { start, stop, lap, reset, getElapsed: elapsed, state };
    render();
  ...
  function initIfPresent(){
    ...
    if (document.getElementById('timer')) {
      if (typeof window.initTimerUI === 'function') {
        try { window.initTimerUI(); console.log('[app] initTimerUI called'); return; } catch(e){ console.error('[app] initTimerUI error', e); }
      }
      bindTimerFallback();
    }
    if (document.getElementById('stopwatch')) {
      if (typeof window.initStopwatchUI === 'function') {
        try { window.initStopwatchUI(); console.log('[app] initStopwatchUI called'); return; } catch(e){ console.error('[app] initStopwatchUI error', e); }
      }
  ...
  try {
    const mo = new MutationObserver((muts)=>{
      for(const m of muts){
        if (m.addedNodes && m.addedNodes.length) { setTimeout(initIfPresent, 50); break; }
      }
    });
  ...
  document.addEventListener('DOMContentLoaded', ()=> setTimeout(initIfPresent, 120));
...
const TAGS = { work:{label:'Work', color:'#2b5fff'}, hobby:{label:'Hobby', color:'#ffb66b'}, study:{label:'Study', color:'#28d37b'} };

function loadFtTasks(){
  try { return JSON.parse(localStorage.getItem('ft_tasks') || '[]'); }
  catch(e){ return []; }
}
function normalizeTag(t){ t = (t||'').toString().trim().toLowerCase(); return ['work','hobby','study'].includes(t)?t:'work'; }

function rangeOf(type){
  const now = new Date(); const s = new Date(now), e = new Date(now);
  if(type==='day'){ s.setHours(0,0,0,0); e.setHours(23,59,59,999); }
  else if(type==='week'){ const day = (now.getDay()+6)%7; s.setDate(now.getDate()-day); s.setHours(0,0,0,0); e = new Date(s); e.setDate(s.getDate()+6); e.setHours(23,59,59,999); }
  else { s.setDate(1); s.setHours(0,0,0,0); e = new Date(s.getFullYear(), s.getMonth()+1, 0); e.setHours(23,59,59,999); }
  return [s,e];
}

function aggregate(rangeType){
  const [s,e] = rangeOf(rangeType);
  const list = (loadFtTasks()||[]).map(t=>({ ...t, tag: normalizeTag(t.tag) })).filter(t=> t.date && (new Date(t.date) >= s && new Date(t.date) <= e));
  ...
  const goal = GOAL_PER_DAY_MIN * (rangeType==='day'?1:(rangeType==='week'?7: (new Date(new Date().getFullYear(), new Date().getMonth()+1,0).getDate())));
...
function renderTotalTime(rangeType){
  const host = document.getElementById('totalTimeChart'); if(!host) return;
  host.innerHTML = '';
  const { list } = aggregate(rangeType);
  const bucket = new Map();
  list.forEach(t=>{ const k = t.date.slice(0,10); bucket.set(k, (bucket.get(k)||0) + (+t.mins||0)); });
  const keys = Array.from(bucket.keys()).sort();
  if(keys.length===0){ host.innerHTML = '<div style="padding:10px;color:#889">데이터 없음</div>'; return; }
  const max = Math.max(60, ...Array.from(bucket.values()));
  keys.forEach(k=>{
    const m = bucket.get(k);
    const el = document.createElement('div');
    el.className = 'bar';
    el.innerHTML = `<i style="height:${(m/max*100).toFixed(1)}%"></i><b>${m}m</b>`;
    host.appendChild(el);
  });
}

function renderRatio(rangeType){
  const host = document.getElementById('ratioChart'); if(!host) return;
  ...
  if(donut) donut.style.background = `conic-gradient(${TAGS.work.color} 0 ${w}%, ${TAGS.hobby.color} ${w}% ${w+h}%, ${TAGS.study.color} ${w+h}% 100%)`;
  host.querySelector('li[data-tag="work"] b').textContent  = `${byTag.work||0}m`;
  host.querySelector('li[data-tag="hobby"] b').textContent = `${byTag.hobby||0}m`;
  host.querySelector('li[data-tag="study"] b').textContent = `${byTag.study||0}m`;
}

function renderGoal(rangeType){
  const host = document.getElementById('goalChart'); if(!host) return;
  ...
  if(txt) txt.innerHTML = `<b>${pct}%</b> (${Math.round(totalMin)}m / ${goal}m)`;
}
...
function renderAllCharts(){
  const getR = id => document.querySelector(`#${id}`)?.closest('.chart-card')?.querySelector('.chart-tabs .on')?.dataset.range || 'day';
