...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function saveTasksToStorage(tasks) {
    try {
      ...
      window.dispatchEvent(
        new CustomEvent('ft-tasks-debug', {
          detail: { ts: Date.now(), count: (tasks || []).length },
        })
      );
    } catch (e) {
...
(function () {
      ...
      if (typeof orig === 'function') {
        window[fnName] = function (...args) {
          ...
          try {
            ...
            console.error(fnName + ' after error', e);
          }
          return res;
        };
        return true;
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    const addBtns = document.querySelectorAll(
      '[data-action="add-todo"], .add-todo, #addTodoBtn'
    );
    addBtns.forEach((btn) => {
...
(function () {
  ...
  function parseDomTasks() {
    ...
    const tasks = Array.from(nodes).map((node, idx) => {
      const id =
        node.dataset.id || 'dom-' + (node.id || Date.now()) + '-' + idx;
      const time = escapeText(node.querySelector('.time-display'));
      const mins = parseDuration(
        escapeText(node.querySelector('.duration-badge'))
      );
      const title = escapeText(node.querySelector('.todo-text'));
      const tagEl = node.querySelector('.category-tag');
      let tag = 'work';
      if (tagEl) {
        const cls = Array.from(tagEl.classList).find(
          (c) => c.startsWith('category-') && c !== 'category-tag'
        );
        if (cls) tag = cls.replace('category-', '');
        else tag = tagEl.textContent.trim().toLowerCase();
      }
      const done =
        node.querySelector('.todo-checkbox')?.classList.contains('checked') ||
        node.querySelector('.todo-text')?.classList.contains('completed') ||
        false;
    ...
    for (const m of muts) {
      if (
        ...
        if (
          all.some(
            (n) =>
              (n.nodeType === 1 && n.matches && n.matches('.todo-item')) ||
              (n.querySelector && n.querySelector('.todo-item'))
          )
        )
          relevant = true;
        if (m.target && m.target.matches && m.target.matches('.todo-item'))
          relevant = true;
      }
      if (relevant) break;
    }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    document.body.addEventListener('click', (e) => {
      const add =
        e.target.closest &&
        e.target.closest('[data-action="add-todo"], .add-todo, #addTodoBtn');
...
(function () {
  ...
  function parseTodoNode(node, idx) {
    ...
    let tag = 'work';
    if (tagEl) {
      const found = Array.from(tagEl.classList || []).find((c) =>
    ...
    const done =
      !!node.querySelector('.todo-checkbox.checked') ||
      !!(titleEl && titleEl.classList.contains('completed'));
  ...
  const observer = new MutationObserver((muts) => {
    ...
    for (const m of muts) {
      if (
        ...
        relevant = [...m.addedNodes, ...m.removedNodes].some(
          (n) =>
            (n.nodeType === 1 && n.matches && n.matches('.todo-item')) ||
            (n.querySelector && n.querySelector('.todo-item'))
        );
      ...
      if (
        ...
        m.target &&
        m.target.matches &&
        m.target.matches('.todo-item, .todo-item *')
      )
        relevant = true;
      if (relevant) break;
    }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    document.body.addEventListener(
      ...
      (ev) => {
        if (
          ev.target.closest &&
          (ev.target.closest('.delete-btn') ||
            ev.target.closest('[data-action="add-todo"]') ||
            ev.target.closest('.add-todo') ||
            ev.target.closest('#addTodoBtn'))
        ) {
...
(function () {
  ...
  function collectDomTasks() {
    return Array.from(document.querySelectorAll('.todo-item')).map((n, i) =>
      parseTodoNode(n, i)
    );
  }
  ...
  function syncFtTasks(debounce = 60) {
    ...
    syncTimer = setTimeout(() => {
      try {
        ...
        const norm = (appTasks || [])
          .map((t, i) => {
            ...
            return {
              ...
              tag:
                t.tag ||
                (t.category && String(t.category).toLowerCase()) ||
                'work',
              done: !!t.done,
            };
          })
        ...
        console.error('[TodoList] sync error', e);
      }
    }, debounce);
  }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    try {
      ...
      console.warn('[TodoList] observer fail', e);
    }
    // initial sync
    syncFtTasks(20);
...
function saveTodo() {
  ...
  window.dispatchEvent(
    new CustomEvent('ft-tasks-updated', { detail: existingFtTasks })
  );
